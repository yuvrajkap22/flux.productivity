...
The site is deployed from the `gh-pages` branch generated by `.github/workflows/pages.yml`.
In GitHub repository settings, set Pages source to the `gh-pages` branch if the site is not already using it.

## Vercel Deployment

Flux also supports Vercel as a static deployment target.

- Build command: `npm run build`
- Output directory: `public`
- Analytics and Speed Insights are loaded through the page entry scripts in `index.html` and `login.html`

If you import the project into Vercel, the included `vercel.json` uses the same build pipeline locally and on Vercel.
